...
import afr.tafeltrainer3.client.pages.DemoPage;
import afr.tafeltrainer3.client.pages.DemoSuperuserPage;
import afr.tafeltrainer3.client.pages.FeedbackPage;
import afr.tafeltrainer3.client.pages.LandingPage;
import afr.tafeltrainer3.client.pages.LoginSuperUser;
import afr.tafeltrainer3.client.pages.NewSuperUser;
import afr.tafeltrainer3.client.pages.ParentMenu;
import afr.tafeltrainer3.client.pages.StartPage;
import afr.tafeltrainer3.client.pages.Tafeltrainer3Gui;
import afr.tafeltrainer3.client.pages.Vragenlijst;
import afr.tafeltrainer3.client.pages.Wallet;
import afr.tafeltrainer3.client.shop.Shop;
import afr.tafeltrainer3.shared.FeedbackContainer;
import afr.tafeltrainer3.shared.Gast;
...
public class MainView extends Composite implements ValueChangeHandler
{
	...
	public void handleEvent(DataEvent result)
	{
		...
		if (result instanceof EventUserRetrieved)
		{
			...
			if (event.getUser() != null)
			{
				...
				// this.gui.hpanel3.clear();
				if (this.user.getEmailsuperuser().equals("jessica@tweemasterlisse.nl") && user.isSurveydone() == false)
				{
					this.contentpanel.clear();
					this.contentpanel.add(new Vragenlijst(this));
				}
				if (this.user.getEmailsuperuser().equals("anja@tweemasterlisse.nl") && user.isSurveydone() == false)
				{
					this.contentpanel.clear();
					this.contentpanel.add(new Vragenlijst(this));
				}
	...
	public void clearOpgavenSet()
	{
		...
		OpgavenSet op = gui.getOpgavenSet();
		if(op!=null && op.getOpgavenSet().size()>0)
		{
			client.emptyAndUpdateUser(op.getOpgavenSet(),user.getId(),user.getMoney());
			op.clearOpgavenSet();
		}
