...
import afr.tafeltrainer3.widgets.GaugeWidget;
...
public class Tafeltrainer3Gui_desktop extends Tafeltrainer3Gui
{
	...
	public OpgavenSet opgavenset = new OpgavenSet();
	
	
	public Tafeltrainer3Gui_desktop()
	{
...
//		this.flex1.setWidget(4, 1, spanel2);
	...
	private class Button1ClickHandler implements ClickHandler
	{
		...
		public void onClick(ClickEvent event)
		{
			...
			try
			{
				GWT.log("Button1 clicked");
				client.startQuiz();
				opgavenset = new OpgavenSet(client,main.getUser().getId());
				numpad.number = "";
	...
	private class checkBoxHandler implements ClickHandler
	{
		...
		public void onClick(ClickEvent event)
		{
			...
			if (!(check.getText().equals("moeilijke"))
			...
			if (chka.getValue() == true && (!(check.getText().equals("moeilijke"))))
			{
				chk2.setValue(false);
				chk3.setValue(false);
				chk4.setValue(false);
			...
			//als niets is gecheckt vallen we weer terug op de defaultwaarde
			if(atLeastOneChecked == false)
			{
				boxes[8].setValue(true);
			}
	...
	private class Button2ClickHandler implements ClickHandler
	{
		...
		public void onClick(ClickEvent event)
		{
			try
			{
				GWT.log("Button2ClickHandler");
				client.emptyAndUpdateUser(opgavenset.getOpgavenSet(), main.getUser().getId(), main.getUser().getMoney());
				opgavenset.clearOpgavenSet();
				main.alertwidget.getBox().setText("");
				main.alertwidget.getContentlabel().setText(messages.tafeltrainerpage_geld_bijgeschreven());
	...
	{
		String opg = String.valueOf(opgave.getFactor1()) + " X " + String.valueOf(opgave.getFactor2()) + " = ";
