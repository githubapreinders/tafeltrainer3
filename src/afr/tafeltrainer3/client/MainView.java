...
public class MainView extends Composite implements ValueChangeHandler
{
	...
	//alle pagina's worden hier gemaakt en vanuit hier geupdatet. Het contentpanel bevat deze pagina's,
	//het menu wordt hier ook geinstantieerd. De rpc-calls gaan allemaal via de clientimplementation.
	public MainView() {
...
//Backbuttonkliks worden hier afgehandeld om te voorkomen dat gebruikers van de site af kukelen bij 
//	een klik op "terug"
	@Override
	...
	//hanteert rpc-calls die door de users worden geintieerd : inloggen, gebruikers aanmaken en wijzigen
	...
	public void handleEvent(DataEvent result) 
	{
		...
		if(result instanceof EventVerifyMail)
		{
			...
			else
			{
				...
				//Utilities.alertWidget("", "Verificatie niet gelukt....", messages).center();
		...
		if(result instanceof EventSuperUserRetrieved)
		{
			...
			if(event.getSuperuser()!=null)
			{
				...
				else 
				{
					...
					Utilities.alertWidgetButtonVerifMail("", messages.main_mailadresnognietgeverifieerd(), messages,superuser,this).center();
				}
			}
			else
			{
...
//na het aanmaken van een superuser		
		if (result instanceof EventAddSuperUser)
		{
			...
			if(event.getSuperuser()!=null)
			{
				...
				{
					Utilities.alertWidget("", "Dit E-mailadres bestaat al, probeer een ander e-mailadres", messages).center();
				}
				else 
				{
					Utilities.alertWidget("", "Klik op de link in uw verificatiemail om uw account te activeren.", messages).center();
				}	
...
//usergroepgegevens worden op de eerste administratiepage gezet zodat ze klaar staan om te editen		
...
//Toont en berekent de inhoud van de portemonnee en zet de plaatjes in het het scherm
